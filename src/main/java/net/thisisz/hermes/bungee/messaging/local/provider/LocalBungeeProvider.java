...
public class LocalBungeeProvider implements LocalProvider {
    ...
    public void displayNewChatMessage(UUID player, ServerInfo server, String message) {
        ...
        ComponentBuilder playerName = new ComponentBuilder(user.getDisplayName());
        ComponentBuilder playerPrefix = new ComponentBuilder(user.getPrefix());
        ComponentBuilder finalMessage = new ComponentBuilder("");
        finalMessage = finalMessage.append(carrotL.create()).append(bracketL.color(ChatColor.DARK_GREEN).create())
                .append(serverName.color(ChatColor.GREEN).create()).append(bracketR.color(ChatColor.DARK_GREEN).create())
                .append(playerPrefix.create()).append(playerName.create()).append(carrotR.create()).append(colon.create()).append(message);
